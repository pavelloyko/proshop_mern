...
Sends requests to the n8n WF1 webhook on a timer with different commands.
traffic_percentage follows a sine wave. Optionally sends intentionally
invalid payloads to test guardrails.

Usage:
    python3 simulate_wf1.py --webhook-url http://localhost:5678/webhook/feature-control --api-key XXX
...
def run(
    ...
    while time.time() - start < duration:
        ...
        # Every 7th request — intentionally invalid (guardrail test)
        ...
        try:
            ...
            try:
                ...
                data = {"raw": r.text[:200]} if r.text else {"raw": "(empty)"}
...
def main() -> None:
    ...
    p.add_argument("--include-invalid", action="store_true", help="Send hallucination-test payloads (-50) every 7th request")
    args = p.parse_args()

    if not args.api_key:
        sys.exit("X-API-Key not set: --api-key or env N8N_API_KEY")
